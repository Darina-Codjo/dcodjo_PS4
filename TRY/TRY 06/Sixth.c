//cd Documents/Cours/'Cours de C'/'Projet en C'/Projet/'TRY 06'
#include <sys/types.h>
#include <string.h>
#include <stdlib.h>
#include <stdio.h>
...
void * threadJob(void * args){
    ...
    //on attend la disponibilité du sémaphore : meltButter
    ...
    if(buffer_butter == SIGUSR1 && buffer_eggYolk == SIGUSR2){
        //on relache le semaphore : meltButter
        sem_post(&sem_meltButter);
        //on ne relache PAS ENCORE le semaphore : eggYolk

        fprintf(stderr,"\n****PREPARATION THREAD [%i]****",tid);
        //on attend la disponibilité du sémaphore : preparation
        ...
        if(buffer_waffleIron == SIGUSR2){
            ...
            if(buffer_plate == SIGUSR1){
                ...
                fprintf(stderr,"\n\n WRITE ERROR : [%i] =/= {%i}\n\n", buffer_plate, SIGUSR1);
                pthread_exit(EXIT_FAILURE);
            }
        }
        else{
            fprintf(stderr,"\n\n WRITE ERROR : [%i] =/= {%i}\n\n", buffer_waffleIron, SIGUSR2);
            pthread_exit(EXIT_FAILURE); 
        }
    }
    else{
        fprintf(stderr,"\n\n WRITE ERROR : [%i] pipe_butter, [%i] pipe_eggYolk\n\n",buffer_butter,buffer_eggYolk);
        pthread_exit(EXIT_FAILURE);
    }
...
void readyToEat(){
    tid = pthread_self();
    fprintf(stderr,"!!! Votre gaufre est servie !!!!");
    fprintf(stderr,"\n\n");
...
int main(){
    ...
    for(int i = 0; i < NB_THREAD; i++){
        ...
        if(error != 0){
            fprintf(stderr,"Echec de la creation du thread");
            return EXIT_FAILURE;
        }
    }
